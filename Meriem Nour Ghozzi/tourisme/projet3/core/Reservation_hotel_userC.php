...
$date_Fin=$Reservation_hotel_user->getdate_Fin();
		...
		$sql="SELECT r.id,date_Debut,date_Fin, h.nom,`image`, `lieu`,  `date_disponible_Debut`, `date_disponible_Fin`, `nbr_places` FROM `reservation_hotel_user` r INNER join user u INNER JOIN hotel h where r.id_User = :id_User and r.id_Hotel = :id_Hotel";
		$db = config::getConnexion();
		try{
			$req=$db->prepare($sql);
			$req->bindValue(':id_Hotel',$id_Hotel);
	  ...
	  function affichermesreservation($id_User){
		...
		$sql="SELECT r.id,date_Debut,date_Fin, h.nom,`image`, `lieu`,  `date_disponible_Debut`, `date_disponible_Fin`, `nbr_places` FROM `reservation_hotel_user` r INNER join user u INNER JOIN hotel h where r.id_User = :id_User and r.id_Hotel = h.id GROUP by r.id";
		$db = config::getConnexion();
		try{
			$req=$db->prepare($sql);
