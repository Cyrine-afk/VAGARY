	...
	function reserverVol($Reservation_vol_user)
	{
 	...
 	$db = config::getConnexion();
	  ...
	  function affichermesreservation($id_User){
		...
		$sql="SELECT r.id,date_heur_disponible,depart,destination ,prix,`nbr_places`, `date_Achat` FROM `reservation_vol_user` r INNER JOIN client u INNER JOIN vol v where r.id_Vol = v.id and r.id_User = :id_User GROUP by r.id ";
		$db = config::getConnexion();
		try{
			$req=$db->prepare($sql);
	  ...
	  function afficherlesreservation_vol($id_Vol){
		...
		$sql="SELECT r.id,u.nom as nomuser,depart,destination, prix,`prenom`, `mail`,date_heur_disponible, `nbr_places`, `date_Achat` FROM `reservation_vol_user` r INNER JOIN client u INNER JOIN vol v where r.id_Vol = :id_Vol and r.id_User = u.id GROUP by r.id";
		$db = config::getConnexion();
		try{
			$req=$db->prepare($sql);
			$req->bindValue(':id_Vol',$id_Vol);
