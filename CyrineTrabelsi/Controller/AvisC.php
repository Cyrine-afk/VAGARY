<?PHP
	include $_SERVER['DOCUMENT_ROOT'].'/projet/VAGARY/CyrineTrabelsi/controller/CommunauteC.php';
	...
	class AvisC {
		...
		function ajouterAvis($avis){
			...
			$sql="insert into aviscom (text_avis,client) values (:text_avis,:client) ";
		...
		function afficherAvis(){
			...
			$sql='SELECT id_avis,text_avis,client,nom_client,prenom_client,date_nai_client,img_client FROM aviscom inner join client on aviscom.client=client.id_client ';
			$db = config::getConnexion();
			try{
		...
		function supprimeravis($id_avis){
			$sql="DELETE FROM aviscom WHERE id_avis= :id_avis";
			$db = config::getConnexion();
			$req=$db->prepare($sql);
		...
		function recupererAvis1($id_avis){
			$sql="SELECT * from aviscom where id_avis=$id_avis";
			$db = config::getConnexion();
			try{
				$query=$db->prepare($sql);
				$query->execute();
